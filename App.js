...
- Permission Policy: For hardware/security missions, mention risk and ask "Shall I engage?"
- JSON Action: Reply + [MODE: TYPE] + JSON Action.
...
export default function App() {
    ...
    if (!isSentinelOn) {
      try {
        const options = {
          ...
          taskDesc: 'Monitoring for wake word...',
        ...
        startSentinel();
      } catch (e) { Alert.alert("Sentinel Error", e.message); }
    } else {
  ...
  const setupLocalLLM = async () => {
    ...
    } catch (e) { console.log("[FRIDAY] Local Brain Error:", e.message); }
  };
