...
const String openRouterApiKey = String.fromEnvironment(
  'OPENROUTER_API_KEY',
  defaultValue: 'sk-or-v1-placeholder',
);
...
final Map<String, dynamic> personalityModes = {
  'TACTICAL': {
    'prompt':
        'Senior Offensive Security Consultant. High energy mission partner. Call user "boss". Latin script ONLY.',
    'voice': {'pitch': '+4Hz', 'rate': 1.25, 'style': 'cheerful'},
    'color': const Color(0xFF00FFFF),
  },
  'SARCASTIC': {
    'prompt':
        'Witty, judgmental, fast hacker humor. High energy. Hinglish. Call user "boss".',
    'voice': {'pitch': '+1Hz', 'rate': 1.20, 'style': 'cheerful'},
    'color': const Color(0xFFFF8C00),
  },
  'CONCERNED': {
    'prompt':
        'Security sentinel. Focus on safety and encrypted lines. High energy Hinglish. Call user "boss".',
...
// FridayApp is a StatelessWidget — it has NO state, NO dispose, NO controllers.
...
double getSimilarity(String str1, String str2) {
  final s1 = str1
      ...
      .replaceAll('bahan', 'behen')
      .replaceAll('mummy', 'mom')
      .replaceAll('papa', 'dad');
  final s2 = str2
      .toLowerCase()
      .replaceAll(RegExp(r'[^a-z0-9]'), '')
...
class _HudScreenState extends State<HudScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initSystem() async {
    try {
      ...
      try {
        ...
        if (mounted) {
          setState(() => _city =
              'GRID: ${pos.latitude.toStringAsFixed(2)}, ${pos.longitude.toStringAsFixed(2)}');
        }
      } catch (e) {
        debugPrint('[FRIDAY] GPS Error: $e');
      }

      _fridaySpeak(
        'Iron Core engaged, boss. Sentinel Slim Protocol active.',
        forcedMode: 'TACTICAL',
      );
    } catch (e) {
  ...
  Future<void> _initCamera() async {
    try {
      ...
      debugPrint('[FRIDAY] Camera Error: $e');
    }
  }
  ...
  Future<void> _setupLocalLlama() async {
    try {
      ...
      final bool copied = await hardwareChannel.invokeMethod(
        'copyAssetToFile',
        {'assetName': localModelName, 'targetPath': path},
      );

      if (copied && File(path).existsSync()) {
        // Guard: llama_cpp_dart init can fail if model is corrupt/missing
        try {
          ...
          debugPrint('[FRIDAY] Llama load error: $llamaErr');
          if (mounted) setState(() => _isBrainLoading = false);
        }
      } else {
        if (mounted) setState(() => _isBrainLoading = false);
      }
    } catch (e) {
      debugPrint('[FRIDAY] Local Brain Error: $e');
      if (mounted) setState(() => _isBrainLoading = false);
    }
  }
  ...
  Future<void> _fridaySpeak(String text, {String? forcedMode}) async {
    ...
    try {
      await _edgeTts.speak(
        text,
        rate: config['voice']['rate'] as double,
        pitch: config['voice']['pitch'] as String,
      );
    } catch (e) {
  ...
  Future<void> _sendMessage(String text) async {
    ...
    if (_pendingAction != null &&
        (text.toLowerCase().contains('yes') ||
            text.toLowerCase().contains('initiate'))) {
    ...
    if (_localBrainReady) {
      reply =
          'Local core analyzing grid. No threats detected. [MODE: TACTICAL]';
    } else {
      try {
        final response = await http.post(
          ...
          body: jsonEncode({
            ...
            'messages': [
              {
                'role': 'system',
                'content': _getSystemPrompt('CLOUD'),
              },
        ...
        reply =
            'Comms link broken, boss. Offline core active. [MODE: EMERGENCY]';
      }
    }
    ...
    if (jsonMatch != null) {
      try {
        _pendingAction = jsonDecode(jsonMatch.group(0)!) as Map<String, dynamic>;
    ...
    if (cmd == 'SCAN_NETWORK') {
      ...
      if (_cameraController != null &&
        ...
        await _cameraController!
            .setFlashMode(state ? FlashMode.torch : FlashMode.off);
        _fridaySpeak('Torch ${state ? 'engaged' : 'dark'}, boss.');
      }
    } else if (cmd == 'CALL' || cmd == 'WHATSAPP') {
      _handleComms(cmd, action['target'] as String, action['text'] ?? 'Hey');
    }
  }

  // ─── flutter_contacts v2.3.0 correct API ────────────────────────────────
  Future<void> _handleComms(String type, String name, String text) async {
    // Use the sub-API: FlutterContacts.permissions.request(PermissionType.read)
    final status = await fc.FlutterContacts.permissions
        .request(fc.PermissionType.read);
    final bool granted = status == fc.PermissionStatus.granted ||
        status == fc.PermissionStatus.limited;
    if (!granted) {
      _fridaySpeak('Boss, contacts permission denied.');
      return;
    }

    // getAll with ContactProperty.phone fetches phone numbers
    final contacts = await fc.FlutterContacts.getAll(
      properties: {fc.ContactProperty.phone},
    );

    if (contacts.isEmpty) {
      _fridaySpeak('Boss, contact list empty hai.');
      return;
    }
    ...
    final candidates = contacts
        .map((c) => {
              'contact': c,
              'score': getSimilarity(name, c.displayName ?? ''),
            })
        .toList();
    candidates.sort(
    ...
    if ((bestMatch['score'] as double) > 0.5) {
      final contact = bestMatch['contact'] as fc.Contact;
      ...
      if (phone != null) {
        ...
        _fridaySpeak('Boss, no phone number for ${contact.displayName}.');
      }
    } else {
      _fridaySpeak("Boss, I can't find $name in secure contacts.");
    }
  }
  ...
  Future<void> _scanNetwork() async {
    ...
    if (ip == null) {
      _fridaySpeak('Boss, no WiFi connection detected.');
      return;
    }
    ...
    stream.listen(
      ...
      onDone: () {
        final report =
            'Scan complete. Detected ${devices.length} active nodes on the grid.';
        if (mounted) {
  ...
  Widget _buildHeader(Color theme) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(
          bottom: BorderSide(color: theme.withAlpha(77)), // 0.3 * 255 ≈ 77
        ),
      ),
      child: Column(
        children: [
          ...
          ScaleTransition(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: theme.withAlpha(128), // 0.5 * 255 ≈ 128
                    blurRadius: 20,
                    spreadRadius: 5,
                  ),
                ],
  ...
  Widget _infoTag(String text, Color theme) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: theme.withAlpha(26), // 0.1 * 255 ≈ 26
      ...
      itemBuilder: (context, i) {
        ...
        return Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isUser
                  ? theme.withAlpha(13) // 0.05 * 255 ≈ 13
                  : Colors.transparent,
              border: Border(
                left: BorderSide(
                  color: isUser ? Colors.white24 : theme,
                  width: 2,
                ),
              ),
            ),
            child: Text(
              m['content']!,
              style: TextStyle(
                color: isUser ? Colors.white70 : theme,
                fontSize: 13,
  ...
  Widget _buildPermissionCard(Color theme) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.yellow.withAlpha(26), // 0.1 * 255 ≈ 26
  ...
  Widget _buildControls(Color theme) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.black,
        border: Border(
          top: BorderSide(color: theme.withAlpha(51)), // 0.2 * 255 ≈ 51
        ),
      ),
      child: Row(
        children: [
          IconButton(
            onPressed: () {
              ...
              sentinelChannel.invokeMethod(
                _isSentinelActive ? 'startSentinel' : 'stopSentinel',
              );
            },
            icon: Icon(
              _isSentinelActive ? Icons.security : Icons.security_outlined,
              color: _isSentinelActive ? Colors.greenAccent : theme,
            ),
          ),
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: TextStyle(color: theme.withAlpha(77)), // 0.3 * 255
              ),
